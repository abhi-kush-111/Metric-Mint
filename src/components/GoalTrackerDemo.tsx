...
const INITIAL_GOALS = [
  { id: 1, name: "LAUNCH MVP", goal: 28, days: Array(28).fill(false) },
  { id: 2, name: "READ 12 BOOKS", goal: 28, days: Array(28).fill(false) },
  { id: 3, name: "SAVE $5000", goal: 28, days: Array(28).fill(false) },
  { id: 4, name: "RUN A MARATHON", goal: 28, days: Array(28).fill(false) },
  { id: 5, name: "LEARN SPANISH", goal: 28, days: Array(28).fill(false) },
  { id: 6, name: "START A BUSINESS", goal: 28, days: Array(28).fill(false) },
  { id: 7, name: "TRAVEL TO JAPAN", goal: 28, days: Array(28).fill(false) },
];
...
export default function GoalTrackerDemo() {
  ...
  // Calculations
  const totalGoal = goals.reduce((sum, h) => sum + h.goal, 0);
  ...
  const dailyCompletion = Array(daysInMonth)
    .fill(0)
    .map((_, dayIdx) => {
      return goals.filter((h) => h.days[dayIdx]).length;
    });
  ...
  const weeklyStats = weeks.map((w) => {
    ...
    const goal = goals.length * days.length;
    return {
      completed,
      goal,
      left: goal - completed,
  ...
  const week5Goal = goals.length * week5Days;
  const week5Percent =
